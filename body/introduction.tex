...
\begin{enumerate}[leftmargin=80pt]
    ...
    \item Minimize Installation Requirements: Designed to process standard video feeds, minimizing the need for costly sensor installations.
    using campus demo footage.

    \item Parking Administrative Portal: A web-based portal that allows
    admins to view the parking spot detection model output and manually draw, or delete parking
    spot boundaries.
\end{enumerate}
...
This project is currently in the Proof of Concept (POC) phase to test the feasibility of using computer vision for campus parking. During this stage, the team focuses on two core areas:
\begin{itemize}
    \item Data Source: Instead of connecting to live campus cameras, the system uses recorded video  captured manually at Kasetsart University parking area and footage from the internet as a data source for demostration.
    
    \item System Testing: The models will be working in a simulated environment, processing the video data to determine parking availability and provide a information output that can be used for the mobile application.
\end{itemize}

By using recorded demo footage, the team can safely test the software's accuracy and the app's user interface without needing permanent hardware installation or live network access during development.